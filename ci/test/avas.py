...
THRESHOLD_VIR = getattr(__config__, 'mcscf_avas_threshold_vir', 0.1)
...
def kernel(mf, aolabels, threshold_occ=THRESHOLD_OCC, threshold_vir=THRESHOLD_VIR, 
    ...
    if openshell_option == 2:
        ...
        log.info('Option 3: thresholds(OCC,VIR) %s %s, num open shell %d', threshold_occ, threshold_vir, mol.spin)
        ncas_occ = (wocc > threshold_occ).sum()
        nelecas = (mol.nelectron - ncore * 2) - (wocc < threshold_occ).sum() * 2
        if ncore > 0: mofreeze = mo_coeff[:,:ncore]
        mocore = mo_coeff[:,ncore:docc].dot(u[:,wocc<threshold_occ])
        mocas = mo_coeff[:,ncore:docc].dot(u[:,wocc>threshold_occ])

        wvir, u = numpy.linalg.eigh(sa[(nocc-ncore):,(nocc-ncore):])
        ncas_vir = (wvir > threshold_vir).sum()
        mocas = numpy.hstack((mocas, mo_coeff[:,docc:nocc],
                              mo_coeff[:,nocc:].dot(u[:,wvir>threshold_vir])))
        movir = mo_coeff[:,nocc:].dot(u[:,wvir<threshold_vir])
        ncas = mocas.shape[1]
