...
//JLC->print the most important determinants and their weights
  ...
  for (int root=0; root<schd.nroots; root++) {
    ...
    std::ofstream myfile;
    std::string filename;
    filename = str(boost::format("%s%i%s")%"dets_nroot_"%root%".txt");
    myfile.open(filename);
    for (int i=0; i<static_cast<int>(DetsSize); i++) {
      compAbs comp;
      int m = distance(&prevci(0,0), max_element(&prevci(0,0), &prevci(0,0)+prevci.rows(), comp));
      if ((abs(prevci(m,0))) >= 1e-6) {
#ifdef Complex
        myfile << format("  %18.10e  ") %(abs(prevci(m,0))); 
        myfile << SHMDets[m] << endl;
#else
        myfile << format("  %18.10e  ") %(prevci(m,0)); 
        myfile << SHMDets[m] << endl;
#endif
        ndets += 1;
      }
    ...
    myfile << "NELACTIVE, NDETS, NORBCORE, NORBACTIVE" << endl;
    myfile << nelec << "  " << ndets << "  -1  " << norbs/2 << endl;
    myfile << "COEFFICIENT/ OCCUPIED ACTIVE SPIN ORBITALS" << endl;
    myfile.close();
  }
//JLC->print the most important determinants and their weights
