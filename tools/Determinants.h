...
class Determinant {
 ...
 public:
  ...
  // jlc Prints the determinant
  friend ostream& operator<<(ostream& os, const Determinant& d) {
    char det[norbs];
    d.getRepArray(det);
    for (int i=0; i<norbs/2; i++) {
      if (det[2*i]==false && det[2*i+1] == false) ;
      else if (det[2*i]==true && det[2*i+1] == false) os<<(i+1)<<"  "; 
      else if (det[2*i]==false && det[2*i+1] == true) os<<(-1*(i+1))<<"  ";
      else if (det[2*i]==true && det[2*i+1] == true) os<<(i+1)<<"  "<<(-1*(i+1))<<"  ";
