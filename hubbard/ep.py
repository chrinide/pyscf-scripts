...
#    exit()
    ...
    s = numpy.identity(nsite)
    print "#######################################"
    pop = numpy.einsum('ij,ji->i', dm1, s)
    for ia in range(nsite):
    	symb1 = 'H'
	print ia+1, symb1, pop[ia]

    dm2 = dm2 - numpy.einsum('ij,kl->ijkl',dm1,dm1)
    dm2 = -dm2
    pairs2 = numpy.einsum('ijkl,ij,kl->ik',dm2,s,s)*0.5 # XC
    print "#######################################"
    for ia in range(nsite):
    	symb1 = 'H'
