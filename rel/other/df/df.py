...
class DF4C_(DF):
    ...
    def build(self):
        ...
        if mem_incore < max_memory:
            ...
            raise NotImplementedError("Outcore not available, requested :", mem_incore)  
        return self
    ...
    def get_jk(self, dm, hermi=1, with_j=True, with_k=True,
               direct_scf_tol=getattr(__config__, 'scf_hf_SCF_direct_scf_tol', 1e-13)):
        return df_jk.r4c_get_jk(self, dm, hermi, with_j, with_k)
...
class DF4C(DF):
    ...
    def build(self):
        ...
        mem_now = lib.current_memory()[0]
        if mem_incore < max_memory:
            self._cderi =(r_incore.cholesky_eri(mol, auxmol=auxmol, aosym='s1',
                                                int3c='int3c2e_spinor', verbose=log),
                          r_incore.cholesky_eri(mol, auxmol=auxmol, aosym='s1',
                                                int3c='int3c2e_spsp1_spinor', verbose=log))
        else:
            raise NotImplementedError("Outcore not available, requested :", mem_incore)  
        return self
...
class DF2C(DF):
    ...
    def build(self):
        ...
        max_memory = (self.max_memory - lib.current_memory()[0]) * .8
        mem_incore = nao_pair*naux*3*8/1e6*2 
        mem_now = lib.current_memory()[0]
        if mem_incore < max_memory:
            self._cderi = r_incore.cholesky_eri(mol, int3c='int3c2e_spinor', auxmol=auxmol,
                                                aosym='s1', verbose=log)
        else:
            raise NotImplementedError("Outcore not available, requested :", mem_incore)  
        return self
    ...
    def loop(self):
        if self._cderi is None:
            self.build()
        with addons.load(self._cderi, 'j3c') as ferill:
            naoaux = ferill.shape[0]
            for b0, b1 in self.prange(0, naoaux, self.blockdim):
                erill = numpy.asarray(ferill[b0:b1], order='C')
                yield erill
