...
def r2c_get_jk(dfobj, dms, hermi=1, with_j=True, with_k=True):
    ...
    tao = mol.tmap()
    n2c = dms.shape[0]
    cderi_ll = dfobj._cderi.reshape(-1,n2c,n2c)
    def fjk(dm):
        vj = numpy.zeros((n2c,n2c), dtype=dm.dtype)
        vk = numpy.zeros((n2c,n2c), dtype=dm.dtype)
        rho = numpy.dot(dfobj._cderi, dm.T.reshape(-1))
        vj = numpy.dot(rho, dfobj._cderi).reshape(n2c,n2c)
...
def r4c_get_jk(dfobj, dms, hermi=1, with_j=True, with_k=True):
    '''Relativistic density fitting JK'''
    t0 = (time.clock(), time.time())
    mol = dfobj.mol
    tao = mol.tmap()
    ...
    cderi_ll = dfobj._cderi[0].reshape(-1,n2c,n2c)
    cderi_ss = dfobj._cderi[1].reshape(-1,n2c,n2c)
    def fjk(dm):
        vj = numpy.zeros((n2c*2,n2c*2), dtype=dm.dtype)
        vk = numpy.zeros((n2c*2,n2c*2), dtype=dm.dtype)
        rho = (numpy.dot(dfobj._cderi[0], dm[:n2c,:n2c].T.reshape(-1))
             + numpy.dot(dfobj._cderi[1], dm[n2c:,n2c:].T.reshape(-1)*c2**2))
        vj[:n2c,:n2c] = numpy.dot(rho, dfobj._cderi[0]).reshape(n2c,n2c)
        vj[n2c:,n2c:] = numpy.dot(rho, dfobj._cderi[1]).reshape(n2c,n2c) * c2**2
...
if __name__ == '__main__':
    ...
    method = density_fit(pyscf.x2c.UHF(mol), 'weigend')
    energy = method.scf()
